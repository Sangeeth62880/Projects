...
const fallbackQuestions: Question[] = [
    {
        questionId: 'nc_1',
        testType: 'number-comparison',
        story: '🦖 Dino has 5 apples. 🐼 Panda has 8 apples. Who has more apples?',
        visualObject: '🍎',
        leftValue: 5,
        rightValue: 8,
        options: ['Dino', 'Panda', 'Same', 'Cannot tell'],
        correctAnswer: 'Panda',
    },
    {
        questionId: 'nc_2',
        testType: 'number-comparison',
        story: '🐰 Bunny collected 12 carrots. 🦊 Fox collected 9 carrots. Who collected fewer?',
        visualObject: '🥕',
        leftValue: 12,
        rightValue: 9,
        options: ['Bunny', 'Fox', 'Same', 'Cannot tell'],
        correctAnswer: 'Fox',
    },
    {
        questionId: 'nc_3',
        testType: 'number-comparison',
        story: '🐶 Dog has 7 bones. 🐱 Cat also has 7 fish. Do they have the same amount?',
        visualObject: '🦴',
        leftValue: 7,
        rightValue: 7,
        options: ['Yes', 'No', 'Dog has more', 'Cat has more'],
        correctAnswer: 'Yes',
    },
    {
        questionId: 'nc_4',
        testType: 'number-comparison',
        story: '🦁 Lion has 15 coins. 🐯 Tiger has 11 coins. Who is richer?',
        visualObject: '💰',
        leftValue: 15,
        rightValue: 11,
        options: ['Lion', 'Tiger', 'Same', 'Cannot tell'],
        correctAnswer: 'Lion',
    },
    {
        questionId: 'nc_5',
        testType: 'number-comparison',
        story: '🐘 Elephant has 20 peanuts. 🐵 Monkey has 25 bananas. Who has more treats?',
        visualObject: '🥜',
        leftValue: 20,
        rightValue: 25,
        options: ['Elephant', 'Monkey', 'Same', 'Cannot tell'],
        correctAnswer: 'Monkey',
    },
];

export default function NumberTestPage() {
        ...
        const loadQuestions = async () => {
            ...
            try {
                if (sessionId && ageGroup) {
                    const response = await api.getQuestions('number-comparison', sessionId, ageGroup);
                    ...
                    startTest('number-comparison', mappedQuestions);
                    addLog("Questions Loaded");
                } else {
                    startTest('number-comparison', fallbackQuestions);
                    addLog("Fallback Questions Loaded");
                }
            } catch (error) {
                console.error('Failed to load questions:', error);
                startTest('number-comparison', fallbackQuestions);
                addLog("Error loading questions");
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const proceedToNext = useCallback(async () => {
        ...
        if (currentQuestionIndex < questions.length - 1) {
            ...
            completeTest();
            router.push('/test/arithmetic-test');
            addLog("Test Completed -> Arithmetic");
        }
    ...
    const handleSubmit = useCallback(async () => {
        ...
        try {
            ...
            const fallbackFeedback = isCorrect
                ...
                : `Great try! The answer was ${currentQuestion.correctAnswer}. You're learning! ⭐`;
    ...
    const handleVoiceInput = useCallback(async (text: string, isAutoSubmit: boolean = false) => {
        ...
        const matchedOption = currentQuestion.options.find(
            opt => {
                ...
                return lowerText === lowerOpt || lowerText.includes(lowerOpt);
            }
        );
            ...
            if (isAutoSubmit) {
                ...
                try {
                    ...
                    const fallbackFeedback = isCorrect
                        ...
                        : `Nice try! The answer was ${currentQuestion.correctAnswer}. You're learning! ⭐`;
                    setAiFeedback(fallbackFeedback);
                    await speakText(fallbackFeedback);
                }
    ...
    }, [currentQuestion, showResult, handleSelectAnswer, submitAnswer, speakText, proceedToNext, addLog]);
        ...
        return (
            <main className="min-h-screen flex items-center justify-center">
                ...
                >
                    ...
                    <p className="text-xl text-gray-600 mt-4">Getting your questions ready... 🎮</p>
                </motion.div>
            </main>
        );
    }
